...
namespace StudentsManagement.Client.Services
{
    public class StudentService : IStudentRepository
    {
        ...
        {
            var newstudent = await _httpClient.DeleteAsync($"api/Student/Delete-Student/{studentId}");
            var response = await newstudent.Content.ReadFromJsonAsync<Student>();
            return response;
        }
        ...
        {
            var singlestudent = await _httpClient.GetAsync($"api/Student/Single-Students/{studentId}");
